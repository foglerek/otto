      ...
      <div className="stack sidebar-stack">
        <section className="hero-card">
          <div className="hero-topline">
            ...
            <div className="prompt-actions"><span className={`badge ${statusBadgeClass(props.state.liveStreamStatus)}`}>stream {props.state.liveStreamStatus}</span></div>
          </div>
        ...
        <div className="panel stack panel-tight"><div className="toolbar"><div><p className="eyebrow">Repository</p><p className="mono repo-path">{dashboard.repoPath}</p></div><button className="button" onClick={props.onRefresh}>Refresh</button></div><p className="subtle">Config</p><p className="mono repo-path">{dashboard.configPath}</p><div className="detail-badges"><span className="badge">onboarding {dashboard.onboardingStatus || "missing"}</span><span className="badge">subagents {dashboard.subagentsEnabled ? "enabled" : "disabled"}</span></div></div>
        <JobsPanel state={props.state} />
        <div className="panel stack panel-tight"><div><p className="eyebrow">Create ticket</p><p className="subtle">Spin up new work directly from the browser.</p></div><textarea className="text-input" rows={5} value={props.state.ticketDraft} onChange={(event) => props.onTicketDraftChange(event.target.value)} placeholder="Describe the work you want Otto to tackle." /><button className="button button-primary" disabled={props.state.isCreatingTicket} onClick={props.onCreateTicket}>{props.state.isCreatingTicket ? "Creating..." : "Create ticket"}</button></div>
        <div className="panel stack panel-tight"><div><p className="eyebrow">Ingest external ticket</p><p className="subtle">Paste or upload markdown and normalize it into Otto’s ticket model.</p></div><input className="file-input" type="file" disabled={props.state.isIngestingTicket} onChange={(event) => { const file = event.target.files?.[0]; if (file) props.onIngestFile(file); }} /><input className="text-line-input mono" type="text" value={props.state.ingestSourceName} disabled={props.state.isIngestingTicket} onChange={(event) => props.onIngestSourceNameChange(event.target.value)} /><textarea className="text-input" rows={7} value={props.state.ingestDraft} onChange={(event) => props.onIngestDraftChange(event.target.value)} placeholder="# Imported ticket&#10;&#10;Paste or type external ticket content here." /><button className="button button-primary" disabled={props.state.isIngestingTicket} onClick={props.onIngestTicket}>{props.state.isIngestingTicket ? "Ingesting..." : "Ingest external ticket"}</button></div>
        <div className="panel stack panel-tight"><div><p className="eyebrow">Ticket inventory</p><p className="subtle">Start browser-driven runs directly from the managed queue.</p></div><div className="ticket-list">{dashboard.tickets.map((ticket) => <div className="ticket-row" key={ticket.ticketId}><div><strong className="mono">{ticket.ticketId}</strong></div>{ticket.hasRun ? <span className="badge">started</span> : <button className="button button-secondary" disabled={isRunBusy(props.state.controlPlane, ticket.ticketId)} onClick={() => props.onStartRun(ticket.ticketId)}>Start</button>}</div>)}</div></div>
      </div>
      <div className="panel stack panel-tight"><div><p className="eyebrow">Runs</p><p className="subtle">Active and historical workflow sessions.</p></div><div className="runs-list">{dashboard.runs.map((run) => <button className={`run-row${run.runId === props.state.selectedRunId ? " active" : ""}`} key={run.runId} onClick={() => props.onSelectRun(run.runId)}><div className="run-row-title"><strong>{run.ticketSlug || run.runId}</strong><span className={`badge ${statusBadgeClass(run.processStatus)}`}>{run.processStatus}</span></div><p className="subtle mono">{run.runId}</p><div className="badge-row"><span className="badge">{run.phase || "unknown"}</span><span className="badge">queue {run.taskQueueLength}</span>{run.needsUserInput ? <span className="badge waiting">waiting</span> : null}</div></button>)}</div></div>
    </aside>
  );
}
